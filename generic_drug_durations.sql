-- This query extracts durations of {drug_name:s} administration
...
DROP MATERIALIZED VIEW IF EXISTS drugs.{drug_name:s}_DURATIONS;
CREATE MATERIALIZED VIEW drugs.{drug_name:s}_DURATIONS as
...
(
  select
    ...
    , max(case when itemid {cv_itemid_test:s} then 1 else 0 end) as vaso -- {drug_name:s}

    -- the 'stopped' column indicates if a vasopressor has been disconnected
    , max(case when itemid {cv_itemid_test:s}       and stopped in ('Stopped','D/C''d') then 1
          else 0 end) as vaso_stopped

    , max(case when itemid {cv_itemid_test:s} and {cv_vaso_null:s} is not null then 1 else 0 end) as vaso_null
    , max(case when itemid {cv_itemid_test:s} then {cv_vaso_rate:s} else null end) as vaso_rate
    , max(case when itemid {cv_itemid_test:s} then {cv_vaso_amount:s} else null end) as vaso_amount

  from mimiciii.{cv_table:s}
  where itemid {cv_itemid_test:s}
...
(
  ...
  where itemid {mv_itemid_test:s} -- {drug_name:s}
